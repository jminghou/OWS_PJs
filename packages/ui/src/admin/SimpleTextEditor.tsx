...
const editorStyles = `
  ...
  .simple-editor a {
    color: hsl(var(--admin-accent-600));
      ...
      {items.map((item, i) => (
        <button
          ...
          className={`p-2 rounded hover:bg-slate-700 transition-colors ${
            item.isActive ? 'text-admin-accent-500' : 'text-white'
          }`}
          title={item.title}
        >
          {item.icon}
...
export function SimpleTextEditor({
  ...
  const editor = useEditor({
    extensions: [
      ...
      Link.configure({
        openOnClick: false,
        HTMLAttributes: {
          class: 'text-admin-accent-600 underline cursor-pointer hover:text-admin-accent-800 transition-colors',
        },
      }),
  ...
  return (
    <div className={`w-full bg-white border border-gray-300 rounded-lg p-3 focus-within:ring-2 focus-within:ring-admin-accent-500 focus-within:border-admin-accent-500 ${className}`}>
