...
# =============================================================================
# Newsletter 模組的 migration 鏈（見 docs/MIGRATIONS.md）
# =============================================================================
# 管理 packages/newsletter/models.py 的 newsletter_* 表。它們與 core 的表同住 BLOG schema，
# 所以這條鏈不能像電商那樣「整個 schema 都是我的」—— 只認 NEWSLETTER_TABLES 裡的表名，
# 其餘一律不反射、不比對，避免 autogenerate 把 core 的表當成「要刪掉」。
#
# 版本表：alembic_version_newsletter，與 core / commerce / studio / 站台鏈互不干擾。
# =============================================================================

NEWSLETTER_VERSION_TABLE = 'alembic_version_newsletter'

from packages.newsletter.migrations_manifest import NEWSLETTER_TABLES  # noqa: E402
...
def _include_name(name, type_, parent_names):
    """只反射本鏈擁有的 schema，且表名限 NEWSLETTER_TABLES。"""
    if type_ == 'schema':
        return (name or 'public') in (_OWNED_SCHEMAS or {'public'})
    if type_ == 'table':
        return name in NEWSLETTER_TABLES
    return True
...
def _table_managed(schema, table_name):
    if _OWNED_SCHEMAS and schema not in _OWNED_SCHEMAS:
        return False
    return table_name in NEWSLETTER_TABLES
...
# 多條鏈共存的關鍵：版本表名稱不同。
_SCHEMA_KWARGS['version_table'] = NEWSLETTER_VERSION_TABLE
