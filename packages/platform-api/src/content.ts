...
// 已移除（P2 契約比對）：以下用戶端方法打的端點在 core 後端並不存在，
// 且 Polaris / Claire 都沒有任何呼叫 —— 是會在執行期 404 的死程式碼。
// 若日後真的需要，請先在 core 加路由，再把方法加回來。
//   contentApi.getComments      → GET  /contents/<id>/comments
//   contentApi.getFeatured      → GET  /contents/featured
