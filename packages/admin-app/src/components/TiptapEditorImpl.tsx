 ...
 * 文章編輯器（Tiptap）。
 *
 * ## 為什麼這裡有一份、packages/ui 也有一份
 *
 * packages/ui 的那份直接 import '@/components/admin/MediaBrowser' 與 '@/lib/utils'，
 * 也就是**共用套件反向依賴消費站台的原始碼**：任何要用它的站台，都必須在那兩個
 * 確切路徑放檔案，而且沒有任何地方寫明這件事。第三個站台會直接踩到。
 *
 * 這份把那兩個相依改成正常的注入：MediaBrowser 是套件自己的元件，
 * 圖片 URL 規則走 configureAdminApp（各站命名規則不同，見 ../config.ts）。
 *
 * packages/ui 的那份**凍結不動**，只服務 Claire_Project
 * （見 docs/FROZEN_CONTRACT.md B 節）。等 Claire 遷移到 @ows/admin-app 之後，
 * 那份連同它的反向依賴一起刪除，重複即消失。
 */
import { useEditor, EditorContent } from '@tiptap/react';
import { BubbleMenu } from '@tiptap/react/menus';
import { NodeViewContent, NodeViewWrapper, ReactNodeViewRenderer } from '@tiptap/react';
import { createArticleBaseExtensions } from './articleExtensions';
import Image from '@tiptap/extension-image';
import Placeholder from '@tiptap/extension-placeholder';
import Link from '@tiptap/extension-link';
import BubbleMenuExtension from '@tiptap/extension-bubble-menu';
import CodeBlockLowlight from '@tiptap/extension-code-block-lowlight';
import { lowlight, CODE_LANGUAGES } from './codeLanguages';
...
import { getImageUrl, getGcsImageUrl } from '../config';
