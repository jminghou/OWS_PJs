...
const nextConfig = {
  ...
  // 轉譯 monorepo 內的套件
  transpilePackages: ['@ows/ui', '@ows/ziwei-chart', '@ows/platform-api', '@ows/admin-app', '@ows/content-kit', '@ows/site-kit', '@ows/ziwei-app', '@ows/commerce', '@ows/studio'],
  ...
  images: {
    remotePatterns: [
      ...
      // 正式環境 API 網域：由 NEXT_PUBLIC_API_HOSTNAME 提供（例如 api.example.com）。
  ...
  env: {
    NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5010/api/v1',
  },
  async headers() {
    return [
      {
